function [x, flag, relres, iter, resvec] = bicg(varargin)
%BICG   BiConjugate Gradients Method codistributed array.
%   X = BICG(A,B)
%   X = BICG(A,B)
%   X = BICG(A,B,TOL)
...
%   
%   Example:
%   Using BICG to solve a large system of codistributed array data:
%   spmd
%       n = 1e6; 
%       on = ones(n,1); 
...
%       dA = codistributed(A);
%       db = codistributed(b);
%       x = BICG(dA,db,tol,maxit)
%   end
%   
%   See also BICG, CODISTRIBUTED.
...
%   Copyright 1984-2016 The MathWorks, Inc.
    ...
    if (isBzero)                       % if rhs vector is all zeros
        ...
        if (nargout < 2)
            parallel.internal.flowthrough.itermsg('bicg', tol, maxit, 0, flag, iter, NaN);
        end
        return
    end
    ...
    [x, flag, relres, iter, resvec] = hBicgImpl(workCodistrB, A, b, tol, maxit, M1, M2, x0);
    
    % only display a message if the output flag is not used
    if nargout < 2
        parallel.internal.flowthrough.itermsg('bicg', tol, maxit, length(resvec), flag, iter, relres);
    end
