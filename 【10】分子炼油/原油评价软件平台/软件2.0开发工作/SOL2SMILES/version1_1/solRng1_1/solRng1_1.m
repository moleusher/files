...
function [flag, sml_ts] = solRng1_1(Comp,SOLCN,SOLHN)
sml_ts = 'NaN';
...
if hetero==0
   ...
   if biphenyl
      ...
      elseif br == 0 && me== 0  % 长直链
         ...
         sml_ts = [sml_tc(1:4),longbr,sml_tc(5:end)];
      elseif br == 0 && me~= 0  % 环取代甲基
         longbr =  solrp(R-1); %
         %sml_tc = insertAfter(sml_tc,4,longbr);
         sml_tc = [sml_tc(1:4),longbr,sml_tc(5:end)];
         %sml_ts = insertAfter(sml_tc,15+R-1,'(C)'); % 目前只能添加一个甲基
         sml_ts = [sml_tc(1:14+R),'(C)',sml_tc(15+R:end)];
      ...
      elseif br ~= 0 && me ~= 0  % 长异构链取代  + 环甲基取代
         ...
         sml_ts = [sml_tc(1:13+index),'(C)',sml_tc(14+index:end)];
      end
      return
   end
   %% 联环己烷
   ...
   if naph 
      ...
      elseif N6 == 1 && N4 == 0 && N2 ==0 % 环己烷
         ...
         elseif br == 0 && me~= 0  % 环取代甲基
             ...
             sml_tc = [cychx(1:4),longbr,cychx(5:end)];
             br_len = length(longbr);
             %sml_ts = insertAfter(sml_tc,br_len+6,'(C)'); % 目前只能添加一个甲基
             sml_ts = [sml_tc(1:br_len+6),'(C)',sml_tc(br_len+7:end)];
         elseif br ~= 0 && me == 0  % 长异构链取代  
             longbr =  solri(R,br); % 
             %sml_ts = insertBefore(cychx,1,longbr);
             sml_ts = [longbr,cychx];
         elseif br ~= 0 && me ~= 0  % 长异构链取代 + 环甲基取代 
             ...
             sml_ts = [sml_tc(1:6+index),'(C)',sml_tc(7+index:end)];
         end
      elseif N4~=0 && N2 == 0
