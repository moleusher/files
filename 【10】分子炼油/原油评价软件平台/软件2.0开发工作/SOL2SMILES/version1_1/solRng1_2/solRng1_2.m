...
%先判断分子类型
%%
function [flag,sml_ts,erro] = solRng1_2(Comp,SOLCN,SOLHN)
sml_ts = 'NaN';
...
if hetero==0
   ...
   %% 判断联苯
   biphenyl=((A6==2)&(AA==1)&(N1==0)&(A4+A2+hetero+N6+N5+N4+N3+N2==0)); % 联苯
   bp_meMax = 9; % 最多9个环上取代甲基
   if biphenyl
       sml_tc = 'c1ccc(c2ccccc2)cc1'; % biphenyl core smiles
      if R == 0                       % next 增加支链
         sml_ts = sml_tc;
      elseif br == 0 && me== 0  % 正构直链取代
         sub_long = msRecur(R,'C');
         sml_ts = [sub_long,sml_tc];
      elseif br == 0 && me~= 0  % 环取代甲基
         sub_long = msRecur(R-me,'C');% R-me>=0
         core_s = bipRecur(me); % 构造主链
         sml_ts = [sub_long,core_s];
      elseif br ~= 0 && me == 0  % 长异构链取代  
         sub_branc =  subRecur(R-br,br); % 
         sml_ts = [sub_branc,sml_tc];
      elseif br ~= 0 && me ~= 0  % 长异构链取代  + 环甲基取代
         sub_branc =  subRecur(R-br-me,br); % 
         core_s = bipRecur(me); % 构造主链
         sml_ts = [sub_branc,core_s];
      end
      return
   end
   %% 联环己烷（）
   ...
   if naph 
      ...
      elseif N6 == 1 && N4 == 0 && N2 ==0 % 环己烷
         ...
         elseif br == 0 && me== 0  % 长直链
             sub_long = msRecur(R,'C');
             sml_ts = [sub_long,cychx];
         elseif br == 0 && me~= 0  % 环取代甲基
             core_chex = chexRecur(me);
              sub_long = msRecur(R-me,'C');
              sml_ts = [sub_long,core_chex];
         elseif br ~= 0 && me == 0  % 长异构链取代  
             sub_branc =  subRecur(R-br,br); % 
             sml_ts = [sub_branc,cychx];
         elseif br ~= 0 && me ~= 0  % 长异构链取代 + 环甲基取代 
             sub_branc =  subRecur(R-br-me,br);
             core_chex = chexRecur(me);
             sml_ts = [sub_branc,core_chex];
         end
      elseif N4~=0 && N2 == 0
    ...
    if cyalene
       ...
       elseif N6~=0 && R>=1 && H == -1 && N5==0
           ...
           sml_ts = [sml_tc(1:6),smlp2,sml_tc(7:end)];
    ...
    aromatics = (A6 == 1)&(A4+A2+hetero+N6+N5+N4+N3+N2+N1==0);
    if aromatics 
        sml_tc = 'c1ccccc1';
        if R + me == 0
            sml_ts = sml_tc; % 苯
...
%         sml_ts = '超出类型苉' ;
    ...
    %% 苉
    chrysene = (A6==1 & A4>0 &(A2+hetero+N6+N5+N4+N3+N2+N1==0));
    if chrysene
        sml_ts = chryRecur(A4);
    return
    end
...
%                 sml_ts = insertBefore(sml_ts1,1,'O'); % 构造仲醇  
                sml_ts0 = ['(',sml_tso];
                sml_ts1 = [sml_ts0,')'];
                sml_ts = ['O',sml_ts1];
            end
    ...
    if ketone
        if (A6+A4+A2+N6+N5+N4+N3+N2+N1==0)&&(br == 0)
            ...
            sml_ts = [sml_tc(1:2),'(=O)',sml_tc(3:end)];
           %sml_ts = 'CC(=O)C ';% (=O)
        elseif (A6+A4+A2+N6+N5+N4+N3+N2+N1==0)&&(br ~= 0)
    ...
    if thio 
        if (A6+A4+A2+N6+N5+N4+N3+N2+N1==0)&&(br == 0)&&(H>=1)&&(AA==0)
            if R <3
               ...
               sml_ts = [sml_tc(1:2),'S',sml_tc(3:end)];
            end
            %% 7- 噻吩
        ...
        elseif (A6+A4+A2+N6+N5+N4+N3+N2+N1==0)&&(br ~= 0)&&(H>=1)&&(AA==0)
            ...
            sml_ts = [sml_tc(1:5),'S',sml_tc(6:end)];
...
%             sml_ts = 'S2c1ccccc1CC2'; % 需要双键表示吗 2017-12-22
    ...
    if s_ol
       if (A6+A4+A2+N6+N5+N4+N3+N2+N1==0)&&(br == 0)
           ...
           sml_tc = solp2smlp(R);
           %sml_ts = insertAfter(sml_tc,2,'(S)');
           sml_ts = [sml_tc(1:3),'(S)',sml_tc(4:end)];
       elseif (A6+A4+A2+N6+N5+N4+N3+N2+N1==0)&&(br ~= 0)
           sml_tc = soli2smli(R,br);
           ollen = length(sml_tc);
           %sml_ts = insertAfter(sml_tc,ollen,'S');
           sml_ts = [sml_tc(1:ollen),'S',sml_tc(ollen+1:end)];
       elseif (A6+A4+A2~=0)&&(N6+N5+N4+N3+N2+N1==0)
    ...
    % 有机酸
     ...
     if organic_acid
         if (A6+A4+A2+N6+N5+N4+N3+N2+N1==0)&&(br == 0)
             ...
             sml_ts = [sml_tc(1:slen),'(=O)O',sml_tc(slen+1:end)];
         elseif (A6+A4+A2+N6+N5+N4+N3+N2+N1==0)&&(br ~= 0)
             sml_tc = soli2smli(R,br);
             slen = length(sml_tc);
%              sml_ts = insertAfter(sml_tc,slen,'(=O)O');
    ...
    if icarn >= 4 
        ...
        if brcyc ==0 
            ...
            for i = 1:brn
                ...
                pafin = [pafin(1:2+(i-1)*4),'(C)',pafin(2+(i-1)*4+1:end)];
            end
            % 甲基数量大于
        ...
        elseif brcyc==1 && brsme >0 
            ...
            for i = 1:brsme
               % pafin = insertAfter(pafin,5+(i-1).*4,'(C)');
                pafin = [pafin(1:5+(i-1)*4),'(C)',pafin(5+(i-1)*4+1:end)];
            end
        elseif brcyc >=2
            tert_unit = 'C(C)(C)';
...
%                sx = insertAfter(pt,length(pt)-4,'=');
               pt = [pt(1:length(pt)-1),'=',pt(length(pt):end)];
               sx = [pt(1:length(pt)-4),'=',pt(length(pt)-3:end)];
           else
               pt = solp2smlp(r);
%                pt = insertAfter(pt,length(pt)-1,'=');
%                sx = insertAfter(pt,length(pt)-4,'=');
...
%% 构造苯环上的取代基
function brsub = subr(Rc,brc,bchar)
    if brc == 0 || Rc <= 2
        mrpfin = solp2smlp(R,'C'); % 取代基上无支链
...
%% 构造多环芳烃苉等
    ...
    %% 构造联苯甲基取代
    function biph = bipRecur(bime)
        %bipc = 'c1ccc(c2ccccc2)cc1';
        %bipc = 'c1ccccc1c2ccccc2';
        if  mod(bime,2)==1 && bime <=4 %奇数
            bip1= ar1Recur(bime);
            bip2= ar2Recur(bime-1);
            biph = [bip2(1:end-3),bip1,bip2(end-2:end)];
        elseif mod(bime,2)==1 && bime >4
            bip1= ar1Recur(bime);
            bip2= ar2Recur(bime-1);
            biph = [bip2(1:11),bip1,bip2(12:end)];
        elseif mod(bime,2)==0 && bime <=4%偶数
            bip2= ar2Recur(bime);
            bip1= ar1Recur(bime-1);
            biph = [bip2(1:end-3),bip1,bip2(end-2:end)];
        elseif mod(bime,2)==0 && bime > 4%偶数 
            bip2= ar2Recur(bime);
            bip1= ar1Recur(bime-1);
     ...
     %% 联苯第二个苯环
    function ar1 = ar1Recur(bime) % bime = 1
        bime = (bime+1)/2;
            %ar1 = '(c2c(C)cccc2)';
            ...
            ar1 = ['(c2',artW,artE,'2)'];
            %art = ar1Recur(bime-1);
            %ar1 = '(c2c(C)c(C)c(C)c(C)c2(C))'
    end
    %% 联苯第一个苯环
    function ar2 = ar2Recur(bime) % bime = 偶数
        if mod(bime,2) == 0
            bime = bime/2;
            if bime <=2
                %ar1 = '(c2c(C)cccc2)';
    ...
    %% 构造联环烷基甲基取代
    function bichex = bichexRecur(bime)
        %bipc = 'c1ccc(c2ccccc2)cc1';
        %bipc = 'c1ccccc1c2ccccc2';
        if  mod(bime,2)==1 && bime <=4 %奇数
            bip1= chex1Recur(bime);
            bip2= chex2Recur(bime-1);
            bichex = [bip2(1:end-3),bip1,bip2(end-2:end)];
        elseif mod(bime,2)==1 && bime >4
            bip1= chex1Recur(bime);
            bip2= chex2Recur(bime-1);
            bichex = [bip2(1:11),bip1,bip2(12:end)];
        elseif mod(bime,2)==0 && bime <=4%偶数
            bip2= chex2Recur(bime);
            bip1= chex1Recur(bime-1);
            bichex = [bip2(1:end-3),bip1,bip2(end-2:end)];
        elseif mod(bime,2)==0 && bime > 4%偶数 
            bip2= chex2Recur(bime);
            bip1= chex1Recur(bime-1);
     ...
     %% 联苯第二个苯环
    function ar1 = chex1Recur(bime) % bime = 1
        bime = (bime+1)/2;
            artW = msRecur(bime,'C(C)');
            artE = msRecur(5-bime,'C');
            ar1 = ['(C2',artW,artE,'2)'];
    ...
    %% 联苯第一个苯环
    function ar2 = chex2Recur(bime) % bime = 偶数
        if mod(bime,2) == 0
            bime = bime/2;
            if bime <=2
                %ar1 = '(c2c(C)cccc2)';
    ...
    %% 环己烷——环取代甲基
    function cyhex = chexRecur(cyme)
               if cyme <= 3 % 邻位开始 br == 0 R 
                   ar_m = msRecur(cyme,'C(C)');
                   ar = msRecur(3-cyme,'C');
                   cyhex = ['C1C',ar_m,ar,'C1'];
               elseif cyme == 4
                   ar_m = msRecur(3,'C(C)');
                   cyhex = ['C1C',ar_m,'C1C'];
               else
                   cyhex = 'C1C(C)C(C)C(C)C(C)C1C';   
               end
    end
